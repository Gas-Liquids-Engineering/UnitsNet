...
// ReSharper restore VirtualMemberNeverOverriden.Global
        ...
        public void From_ValueAndUnit_ReturnsQuantityWithSameValueAndUnit()
        {
            ...
            var quantity29 = Pressure.From(1, PressureUnit.MillimeterOfWaterColumn);
            AssertEx.EqualTolerance(1, quantity29.MillimetersOfWaterColumn, MillimetersOfWaterColumnTolerance);
            Assert.Equal(PressureUnit.MillimeterOfWaterColumn, quantity29.Unit);

            var quantity30 = Pressure.From(1, PressureUnit.Millipascal);
            AssertEx.EqualTolerance(1, quantity30.Millipascals, MillipascalsTolerance);
            Assert.Equal(PressureUnit.Millipascal, quantity30.Unit);

            var quantity31 = Pressure.From(1, PressureUnit.NewtonPerSquareCentimeter);
            AssertEx.EqualTolerance(1, quantity31.NewtonsPerSquareCentimeter, NewtonsPerSquareCentimeterTolerance);
            Assert.Equal(PressureUnit.NewtonPerSquareCentimeter, quantity31.Unit);

            var quantity32 = Pressure.From(1, PressureUnit.NewtonPerSquareMeter);
            AssertEx.EqualTolerance(1, quantity32.NewtonsPerSquareMeter, NewtonsPerSquareMeterTolerance);
            Assert.Equal(PressureUnit.NewtonPerSquareMeter, quantity32.Unit);

            var quantity33 = Pressure.From(1, PressureUnit.NewtonPerSquareMillimeter);
            AssertEx.EqualTolerance(1, quantity33.NewtonsPerSquareMillimeter, NewtonsPerSquareMillimeterTolerance);
            Assert.Equal(PressureUnit.NewtonPerSquareMillimeter, quantity33.Unit);

            var quantity34 = Pressure.From(1, PressureUnit.Pascal);
            AssertEx.EqualTolerance(1, quantity34.Pascals, PascalsTolerance);
            Assert.Equal(PressureUnit.Pascal, quantity34.Unit);

            var quantity35 = Pressure.From(1, PressureUnit.PoundForcePerSquareFoot);
            AssertEx.EqualTolerance(1, quantity35.PoundsForcePerSquareFoot, PoundsForcePerSquareFootTolerance);
            Assert.Equal(PressureUnit.PoundForcePerSquareFoot, quantity35.Unit);

            var quantity36 = Pressure.From(1, PressureUnit.PoundForcePerSquareInch);
            AssertEx.EqualTolerance(1, quantity36.PoundsForcePerSquareInch, PoundsForcePerSquareInchTolerance);
            Assert.Equal(PressureUnit.PoundForcePerSquareInch, quantity36.Unit);

            var quantity37 = Pressure.From(1, PressureUnit.PoundPerInchSecondSquared);
            AssertEx.EqualTolerance(1, quantity37.PoundsPerInchSecondSquared, PoundsPerInchSecondSquaredTolerance);
            Assert.Equal(PressureUnit.PoundPerInchSecondSquared, quantity37.Unit);

            var quantity38 = Pressure.From(1, PressureUnit.TechnicalAtmosphere);
            AssertEx.EqualTolerance(1, quantity38.TechnicalAtmospheres, TechnicalAtmospheresTolerance);
            Assert.Equal(PressureUnit.TechnicalAtmosphere, quantity38.Unit);

            var quantity39 = Pressure.From(1, PressureUnit.TonneForcePerSquareCentimeter);
            AssertEx.EqualTolerance(1, quantity39.TonnesForcePerSquareCentimeter, TonnesForcePerSquareCentimeterTolerance);
            Assert.Equal(PressureUnit.TonneForcePerSquareCentimeter, quantity39.Unit);

            var quantity40 = Pressure.From(1, PressureUnit.TonneForcePerSquareMeter);
            AssertEx.EqualTolerance(1, quantity40.TonnesForcePerSquareMeter, TonnesForcePerSquareMeterTolerance);
            Assert.Equal(PressureUnit.TonneForcePerSquareMeter, quantity40.Unit);

            var quantity41 = Pressure.From(1, PressureUnit.TonneForcePerSquareMillimeter);
            AssertEx.EqualTolerance(1, quantity41.TonnesForcePerSquareMillimeter, TonnesForcePerSquareMillimeterTolerance);
            Assert.Equal(PressureUnit.TonneForcePerSquareMillimeter, quantity41.Unit);

            var quantity42 = Pressure.From(1, PressureUnit.Torr);
            AssertEx.EqualTolerance(1, quantity42.Torrs, TorrsTolerance);
            Assert.Equal(PressureUnit.Torr, quantity42.Unit);
        ...
        public void ToUnit()
        {
            ...
            AssertEx.EqualTolerance(MillimetersOfWaterColumnInOnePascal, (double)millimeterofwatercolumnQuantity.Value, MillimetersOfWaterColumnTolerance);
